...
# def run_options(contract_map=CONTRACT_MAP,
...
#             continue
#
#         # Map expirations to the next 10 NYSE trading days => DTE index 0..9
#         nyse = mcal.get_calendar("NYSE")
#         sched = nyse.schedule(start_date=today, end_date=today + pd.Timedelta(days=20))
#         trading_days = list(sched.index.normalize())[:11]
#         tmap = {d.strftime("%Y-%m-%d"): i for i, d in enumerate(trading_days)}
...
#         df = calculate_greek_exposures_and_tags(df, spot_price)
#
#         # Realized vol blend (fallback to SPY if ticker minute data is empty)
#         def _intraday_rv_yf(symbol: str, window_min: int = 15) -> float:
#             try:
#                 dfi = yf.download(symbol, period="1d", interval="1m", progress=False, auto_adjust=True)
#                 if dfi is None or dfi.empty:
...
#         df_monte_carlo = df_monte[MONTE_CARLO_COLUMNS].copy()
#
#         # === Save per-ticker Excel (8 sheets) ===
#         out_dir = get_folder(ticker, "options")
#         os.makedirs(out_dir, exist_ok=True)
#         filename = os.path.join(out_dir,
#                                 f"{ticker.lower()}_options_data_{timestamp_str}.xlsx")
#
...
#         book.save(filename)
#
#         print(f"Saved {ticker} options → {filename}")
...
#
#     # optional:
#     # setup_logging(today, r"C:\2mdt\2mindt-site\logs\options", timestamp_str)
...
#
#         # === Save per-ticker Excel (8 sheets) ===
#         # out_dir = os.path.join("C:/2mdt/2mindt-site/public", ticker.lower(), f"{ticker.lower()}-options-data")
#         # os.makedirs(out_dir, exist_ok=True)
#         # filename = os.path.join(out_dir, f"{ticker.lower()}_options_data_{timestamp_str}.xlsx")
#         out_dir = get_folder(ticker, "options")
#         os.makedirs(out_dir, exist_ok=True)
#         filename = os.path.join(out_dir,
#                                 f"{ticker.lower()}_options_data_{timestamp_str}.xlsx")
...
#     print("=== Running cowpie options test ===")
#     run_cowpie()
